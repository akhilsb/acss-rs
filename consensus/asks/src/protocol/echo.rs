...
impl Context {
    pub async fn process_asks_echo(
        &mut self,
        ctrbc_msg: CTRBCMsg,
        echo_sender: Replica,
        instance_id: usize,
        ...
        if asks_state.terminated {
            ...
            log::debug!(
                "ASKS {} already terminated, skipping ECHO processing",
                instance_id
            );
            return;
        }
        ...
        if size == self.num_nodes - self.num_faults {
            log::info!(
                "Received n-f ECHO messages for ASKS Instance ID {}, sending READY message",
                instance_id
            );
