...
impl Context {
    pub async fn process_ready_vf(
        ...
        acss_va_context
            .bv_ready_points
            .insert(ready_sender + 1, deser_share);
        ...
        if ready_senders.len() == self.num_faults + 1 {
                ...
                && acss_va_context.verified_hash.unwrap() == root
            {
                ...
                let status =
                    reconstruct_data(&mut shards, self.num_faults + 1, 2 * self.num_faults);
                ...
                if merkle_tree.root() == root {
                    ...
                    let (poly_coeffs, nonce_coeffs, bpoly_coeffs, bnonce_coeffs) =
                        proof_status.unwrap();
                    ...
                    for rep in 0..self.num_nodes {
                        if !acss_va_state.column_shares.contains_key(&rep) {
                            let column_shares = poly_coeffs
                                .iter()
                                .map(|poly| self.large_field_uv_sss.mod_evaluate_at(poly, rep + 1))
                                .collect();
                            acss_va_state.column_shares.insert(
                                ...
                                (
                                    ...
                                    self.large_field_uv_sss
                                        .mod_evaluate_at(&nonce_coeffs, rep + 1),
                                ),
                            );
                            acss_va_state.bcolumn_shares.insert(
                                ...
                                (
                                    self.large_field_uv_sss
                                        .mod_evaluate_at(&bpoly_coeffs, rep + 1),
                                    self.large_field_uv_sss
                                        .mod_evaluate_at(&bnonce_coeffs, rep + 1),
                                ),
                            );
                        }
                    }
            ...
            log::info!(
                "Received n-f READY messages for ACSS Instance ID {}, terminating",
                instance_id
            );
            // Terminate protocol
            ...
            self.terminate(row_secret_shares, root_comm, instance_id)
                .await;
        }
    }
    ...
    pub async fn terminate(
        ...
        log::info!(
            "Terminating ACSS for instance id {}, true_inst_id: {}, sender_party: {}",
            instance_id,
            true_inst_id,
            sender_party
        );

        let shares_ser = shares
            .into_iter()
            .map(|share| share.to_signed_bytes_be())
            .collect();
        let _status = self
            .out_acss_shares
            .send((true_inst_id, sender_party, root_comm, shares_ser))
            .await;
    }
}
