...
impl Context {
    ...
    pub async fn init_verifiable_abort(
        ...
        for _rep in 0..self.num_nodes {
            ...
            // Coefficients
            let bpoly_eval_pts: Vec<(BigInt, BigInt)> = (0..self.num_faults + 1)
                .into_iter()
                .map(|x| BigInt::from(x))
                .zip(bpoly_y_deg_t.clone().into_iter())
                .collect();
            let bpoly_coeffs = self
                .large_field_uv_sss
                .polynomial_coefficients(&bpoly_eval_pts);
            ...
            .zip(column_wise_roots.into_iter())
        {
            ...
            let coeffs_const_size: Vec<Vec<u8>> = self
                .folding_dzk_context
                .gen_dzk_proof(&mut eval_points, &mut trees, coefficients, iteration, root)
                .into_iter()
                .map(|x| x.to_signed_bytes_be())
                .collect();
            ...
            for (g_0_g_1_shares, mt) in eval_points.into_iter().zip(trees.into_iter()) {
                    ...
                    .zip(g_0_g_1_shares.into_iter())
                {
                    dzk_proofs_all_nodes
                        ...
                        .unwrap()
                        .g_0_x
                        .push(g.0.to_signed_bytes_be());
                    dzk_proofs_all_nodes
                        .get_mut(rep)
                        .unwrap()
                        .g_1_x
                        .push(g.1.to_signed_bytes_be());
                    dzk_proofs_all_nodes
                        .get_mut(rep)
                        .unwrap()
                        .proof
        ...
        for (rep, dzk_proofs) in shares_proofs_dzk.into_iter() {
            ...
            let blinding_column_poly: Vec<(LargeFieldSer, LargeFieldSer)> = blinding_y_deg_t[rep]
                ...
                .map(|(share, nonce)| (share.to_signed_bytes_be(), nonce.to_signed_bytes_be()))
                .collect();
    ...
    async fn generate_shares(
        ...
        for secret in secrets {
            ...
            for rep in 0..num_nodes {
                ...
                if rep <= num_faults - 1 {
                    poly_x_deg_2t
                        .extend(large_field_bv_sss.split(share).into_iter().map(|tup| tup.1));
                }
    ...
    pub async fn process_acss_init_vf(
        ...
        let row_shares: Vec<Vec<LargeField>> = shares
            ...
            .map(|x| {
                x.iter()
                    .map(|el| LargeField::from_signed_bytes_be(el))
                    .collect()
            })
            .collect();
        ...
        let blinding_row_shares: Vec<LargeField> = shares
            .blinding_row_poly
            .iter()
            .map(|x| LargeField::from_signed_bytes_be(x.0.clone().as_slice()))
            .collect();
        // Appended
        // Verify commitments
        ...
        for share_poly in shares.column_poly.0 {
            let mut col_shares = share_poly
                .into_iter()
                .map(|el| LargeField::from_signed_bytes_be(el.as_slice()))
                .collect();
    ...
    fn agg_share_poly_dzk_batch(
        ...
        let mut root_mul_lf: Vec<LargeField> = column_wise_roots
            .iter()
            .map(|root| LargeField::from_signed_bytes_be(root) % &self.large_field_uv_sss.prime)
            .collect();
