...
use crate::{DZKProof, LargeField, PointBV, ShamirSecretSharing};
use crypto::{
...
impl FoldingDZKContext {
    ...
    pub fn gen_dzk_proof(
        ...
        let evaluations: Vec<LargeField> = self
            .evaluation_points
            .clone()
            .into_iter()
            .map(|x| {
        ...
        let hashes: Vec<Hash> = evaluations
            .iter()
            .map(|x| do_hash(x.to_bytes_be().as_slice()))
            .collect();
        ...
        let second_half_coeff: Vec<LargeField> = first_half_coeff.split_off(split_point);

        // 3. Calculate evaluation points on both split polynomials
        let g_vals: Vec<(LargeField, LargeField)> = self
            .evaluation_points
            .clone()
            .into_iter()
            .map(|rep: usize| {
        ...
        poly_folded = ShamirSecretSharing::add_polynomials(
            &Polynomial::new(&poly_folded[..]),
            &Polynomial::new(&first_half_coeff[..]),
        )
        .coefficients;
        // for (index, coeff) in (0..first_half_coeff.len())
        //     .into_iter()
        //     .zip(first_half_coeff.into_iter())
        // {
        ...
        return self.gen_dzk_proof(
            eval_points,
            trees,
            poly_folded,
            iteration + 1,
            aggregated_root_hash,
        );
    }

    pub fn verify_dzk_proofs_column(
        &self,
        dzk_roots: Vec<Hash>,
        ...
        for rep in self.evaluation_points.clone().into_iter() {
            if bv_ready_points.contains_key(&rep) {
                ...
                let combined_root = self
                    .hash_context
                    .hash_two(column_share.2.root(), bcolumn_share.2.root());

                let deser_points: Vec<LargeField> = column_share
                    .0
                    .clone()
                    .into_iter()
        ...
        if column_evaluation_points.len() < self.recon_threshold {
            log::error!(
                "Did not receive enough valid points from other parties, abandoning ACSS {}",
                instance_id
            );
            return None;
        }
        ...
        let bpoly_coeffs = self
            .large_field_uv_sss
            .polynomial_coefficients_with_precomputed_vandermonde_matrix(
                &blinding_evaluation_points,
            );
        let bnonce_coeffs = self
            .large_field_uv_sss
    ...
    pub fn verify_dzk_proof(
        ...
        let first_poly: Vec<LargeField> = dzk_poly
            .into_iter()
        ...
        let g_0_pts: Vec<LargeField> = dzk_proof
            ...
            .map(|x| LargeField::from_bytes_be(x.as_slice()).unwrap())
            .collect();
        let g_1_pts: Vec<LargeField> = dzk_proof
            .g_1_x
            .into_iter()
            .rev()
            .map(|x| LargeField::from_bytes_be(x.as_slice()).unwrap())
        ...
        for (index, (g_0, g_1)) in (0..g_0_pts.len())
            .into_iter()
            .zip(g_0_pts.into_iter().zip(g_1_pts.into_iter()))
        {
            ...
            let split_point = *self
                .poly_split_evaluation_map
                .get(&(degree_poly as isize))
                .unwrap() as usize;
            ...
            if !merkle_proof.validate(&self.hash_context)
                ...
                || rev_roots[index] != merkle_proof.root()
            {
                ...
                log::error!(
                    "Merkle root matching: computed: {:?}  given: {:?}",
                    rev_roots[index].clone(),
                    merkle_proof.root()
                );
                log::error!(
                    "Items: {:?}  given: {:?}",
                    merkle_proof.item(),
                    do_hash(point.to_bytes_be().as_slice())
                );
                return false;
            }
        }
        ...
        if point != dzk_share {
            log::error!(
                "DZK Point does not match the first level point {:?} {:?} for {}'s column",
                point,
                dzk_share,
                evaluation_point
            );
    ...
    pub fn verify_dzk_proof_row(
        ...
        for ((ind_roots, first_root), (share, blinding)) in
            (roots.into_iter().zip(column_roots.into_iter()))
        ...
        let mut _rep = 0;
        for ((dzk_proof, first_poly), ((rev_agg_root_vec, rev_root_vec), dzk_share)) in (dzk_proofs
            .into_iter()
            .zip(dzk_polys.into_iter()))
        .zip((rev_agg_roots.into_iter().zip(rev_roots.into_iter())).zip(dzk_shares.into_iter()))
        {
            // These are the coefficients of the polynomial
            //log::info!("DZK verification Hashes {:?} for rep {}", rev_agg_root_vec, rep);
            let first_poly: Vec<LargeField> = first_poly
                .into_iter()
                .map(|x| LargeField::from_bytes_be(x.as_slice()).unwrap())
                .collect();
            let mut degree_poly = first_poly.len() - 1;
